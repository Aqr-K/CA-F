<template>
  <v-card class="mx-auto" width="100%">
    <VToolbar>
      <VTextField ref="inputKeyword" v-model="keyword" label="输入名称搜索" single-line placeholder="电影或电视剧名称" variant="solo"
        prepend-inner-icon="mdi-magnify" flat class="mx-1" :loading="loading" @click:prependInner="searchMedias"
    ...
    <VList v-if="items.length > 0" lines="three">
      <template v-for="(item, i) in items" :key="i">
        <VListItem @click="selectMedia(item)">
          <template #prepend>
            <VImg height="75" width="50" :src="item.poster" aspect-ratio="2/3"
...
const mediaId = defineModel("mediaId")
