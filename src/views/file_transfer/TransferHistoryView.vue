<template>
    <VCard>
            ...
            <VCardTitle>
                <VRow>
                    ...
                    <VCol cols="8" md="6" class="flex">
                        <VCombobox key="search_navbar" v-model="search" :items="searchHintList" class="text-disabled"
                            density="compact" label="搜索目录、状态" prepend-inner-icon="mdi-magnify" variant="solo-filled"
                            single-line hide-details flat rounded clearable />
                    </VCol>
                </VRow>
            </VCardTitle>
        </VCardItem>
        <VDataTableVirtual v-model="selected" :headers="headers" :items="dataList" :loading="loading" density="compact"
            return-object fixed-header show-select loading-text="加载中..." hover :style="tableStyle">
            ...
            <template #item.actions="{ item }">
                <IconBtn>
                    ...
                    <VMenu activator="parent" close-on-content-click>
                        <VList>
                            <VListItem v-for="(menu, i) in dropdownItems" :key="i" variant="plain"
        ...
        <div class="flex items-center justify-end">
            <div class="w-auto">
                <VSelect v-model="itemsPerPage" :items="pageRange" density="compact" variant="solo"
                    style="padding-top: 0 !important;" flat />
            </div>
            <div class="w-auto text-sm">{{ pageTip.begin }}-{{ pageTip.end }} / {{ totalItems }}</div>
            <VPagination v-model="currentPage" show-first-last-page :length="totalPage" @next="currentPage + 1"
                @prev="currentPage - 1">
            </VPagination>
        </div>
    </VCard>
    ...
    <!-- 底部操作按钮 -->
    <span>
        <VFab v-if="selected.length > 0" icon="mdi-trash-can-outline" color="error" location="bottom" size="x-large"
            fixed app appear @click="removeHistoryBatch" :class="{ 'mb-12': appMode }" />
        <VFab v-if="selected.length > 0" :class="appMode ? 'mb-28' : 'mb-16'" icon="mdi-redo-variant" location="bottom"
            size="x-large" fixed app appear @click="retransferBatch" />
    </span>
    <!-- 底部弹窗 -->
    ...
    <!-- 文件整理弹窗 -->
    <FileTransferDialog v-if="redoDialog" v-model="redoDialog" :history_ids="redoIds" @done="transferDone"
...
function getIcon(type: string) {
    ...
    else if (type === 'tv') return 'mdi-television-classic'
...
async function removeHistory(item: TransferHistory) {
    ...
    confirmTitle.value = `确认删除 ${item.title} ${item.season_episode} ?`
