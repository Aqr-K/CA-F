<template>
  <div v-if="!isLoading">
    <v-card title="PlexServer" subtitle="用于生成Strm｜软链接时通知Plex扫库">
      <v-card-text>
        <draggable :list="settings" handle=".cursor-move" delay="100" item-key="id" tag="div"
          ...
          <template #item="{ element, index }: { element: MediaServer; index: number }">
            <v-card color="card">
              ...
              <v-text-field label="地址" v-model="element.host" hint="Plex的地址" persistent-hint
                class="mx-3 my-2"></v-text-field>
              <v-text-field label="API Key" v-model="element.api_key" hint="Plex的API Key" persistent-hint
                class="mx-3 my-2"></v-text-field>
              <v-text-field label="延迟通知" v-model="element.time_delay" hint="延迟通知Plex扫描的时间" persistent-hint
                class="mx-3 my-2"></v-text-field>
              ...
              <v-btn @click="testPlexServer(element)" class="mx-3 my-3">测试</v-btn>
            </v-card>
          </template>
        </draggable>
        <v-alert type="info" variant="tonal" class="mt-2 mb-5">
          <span class="text-[16px]">
            Plex服务器配置排在第一个的将作为默认Plex服务器
          </span>
        </v-alert>

        <v-alert type="info" variant="tonal">
          <span v-html="`
          路径替换：CA和Plex对于同一个目录映射方式相同时可以不填
                    <br><br>

          Plex通知的原理是把生成的Strm｜软链接的路径通知给Plex，让Plex去扫描该路径，如果Strm｜软链接的目录在CA和Plex中的映射不一样，则需要在这里进行替换。因此建议CA和Plex在映射目录时尽量保持一致
          <br><br>
          支持的配置格式（注意空格）：多个配置用;隔开
          <br><br>
          被替换路径1 => 替换路径1;被替换路径2 => 替换路径2;...
          <br><br>
          示例：比如对于/mnt/Symlink在CA中的映射为/mnt/Symlink:/CA/Symlink,在Plex中的映射为/mnt/Symlink:/Plex/Symlink,我们就需要把/CA/Symlink替换为/Plex/Symlink
          <br><br>
          那么路径替换就是：/CA/Symlink => /Plex/Symlink          
          `" class="text-[16px]" />
        </v-alert>
...
async function fetchSyncConfig() {
  try {
    const response: MediaServer[] = await api.get('/system/settings/' + 'plex_server')
...
async function saveConfig() {
  try {
    let data = { settings: settings.value, name: "plex_server" }
...
async function testPlexServer(config: MediaServer) {
  try {
    const response: SaveResponse = await api.post(`/plugin/test_Plex`, config)
    if (response.success) {
      $toast.success(response.message);
    } else {
      $toast.error(response.message);
    ...
    console.error('Error testing Plex server:', error)
  }
}
